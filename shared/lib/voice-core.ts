...
// ── Shared mic stream singleton ───────────────────────────────────────────────
// WebKit2GTK/GStreamer can deadlock when two concurrent getUserMedia calls open
// separate capture pipelines simultaneously (e.g. wake-word + VAD).
// Both useWakeWord and useSimpleVAD call getMicStream() — only ONE getUserMedia
// call ever happens; both hooks get the same MediaStream object.
let _sharedStream:  MediaStream | null = null
let _sharedPending: Promise<MediaStream> | null = null

export async function getMicStream(): Promise<MediaStream> {
  if (_sharedStream?.active) return _sharedStream
  if (_sharedPending) return _sharedPending

  if (typeof navigator === 'undefined' || !navigator.mediaDevices?.getUserMedia)
    throw new Error('getUserMedia not available')

  _sharedPending = Promise.race([
  ...
  return _sharedPending
}

/** Stop the shared stream and clear the singleton (call on page unmount). */
...
// WebKit2GTK blocks HTMLAudioElement.play() even after a user gesture.
// Fetch the blob URL as ArrayBuffer and play via Web Audio API instead.
// onDone is called when playback ends (or fails) — same contract as onended/onerror.
async function _playViaWebAudio(url: string, volume: number, onDone: () => void): Promise<void> {
  let _called = false
  const _once = () => { if (!_called) { _called = true; onDone() } }
  try {
    ...
    try {
      ...
      src.onended = () => { ctx.close().catch(() => {}); _once() }
      src.start(0)
      // Safety: if onended never fires (WebKit2GTK bug), resolve after duration + 2s
      setTimeout(() => { try { src.stop() } catch {} ctx.close().catch(() => {}); _once() },
        (buf.duration + 2) * 1000)
    } catch {
      ctx.close().catch(() => {})
      _once()
...
function _playOrQueue(
  ...
  el.play().catch((err: unknown) => {
    if (err instanceof DOMException && err.name === 'NotAllowedError') {
      // WebKit2GTK autoplay blocked — try Web Audio API before falling back to queue.
      const url = el.src
      if (url) {
        _playViaWebAudio(url, el.volume, onError)
        return  // Web Audio API now owns playback; onError doubles as onDone for AudioQueue
      }
...
export class AudioQueue {
  ...
  private async _next(): Promise<void> {
    ...
    await new Promise<void>((res) => {
      ...
      // Hard 8s cap — prevents per-chunk hangs in WebKit2GTK
      const t = setTimeout(done, 8_000)
      a.onended = () => { clearTimeout(t); done() }
      a.onerror = () => { clearTimeout(t); done() }
      // Pre-prime at 85% so mic restarts before the chunk fully ends
      a.ontimeupdate = () => {
        if (!finished && a.duration > 0 && a.currentTime / a.duration >= 0.85) {
          a.ontimeupdate = null; done()
        }
      }

      a.play()
        .then(() => {
          // WebKit2GTK: play() may resolve silently without actually playing.
          // If loadedmetadata doesn't fire in 3s, fall back to Web Audio API.
          let metaLoaded = false
          const metaTimer = setTimeout(() => {
            if (finished || metaLoaded) return
            console.warn('[AUDIO] loadedmetadata timeout — Web Audio fallback')
            a.pause(); a.src = ''
            a.onended = null; a.onerror = null; a.ontimeupdate = null
            _playViaWebAudio(url, vol, () => { clearTimeout(t); done() })
          }, 3_000)
          a.onloadedmetadata = () => { metaLoaded = true; clearTimeout(metaTimer) }
        })
        .catch((err: unknown) => {
          if (err instanceof DOMException && err.name === 'NotAllowedError') {
            // Autoplay blocked — try Web Audio API (no user-gesture restriction on AudioContext)
            _playViaWebAudio(url, vol, () => { clearTimeout(t); done() })
          } else {
            clearTimeout(t); done()
          }
