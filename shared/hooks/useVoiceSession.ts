...
import { useSimpleVAD } from '@/hooks/useSimpleVAD'
...
// Module-level lock: prevents concurrent startSession() calls in React StrictMode / HMR double-mount.
// Lives outside the hook so it's shared across all instances and survives re-renders.
...
export function useVoiceSession() {
  ...
  // Tracks getUserMedia failures so _startListening can detect the silent
  // no-op that occurs when the VAD instance is in "errored" state — the
  // hook's start() swallows the error and returns without actually listening.
  const vadMicErrorRef = useRef<string | null>(null)
  ...
  // useSimpleVAD replaces the ONNX-based useMicVAD from @ricky0123/vad-react.
  // AudioWorklet (which the ONNX path needs) silently fails in WebKit2GTK/WSL2,
  // leaving the session stuck in a deaf "listening" state. useSimpleVAD uses
  // ScriptProcessorNode + energy RMS — no WASM, no SharedArrayBuffer required.
  const vad = useSimpleVAD({
    // Use the shared singleton stream — same MediaStream as useWakeWord, so only
    // ONE getUserMedia call ever happens in the app (prevents GStreamer deadlock on WSL2).
    getStream: async () => {
      try {
        const stream = await getMicStream()
        vadMicErrorRef.current = null
        return stream
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err)
        console.error('[VAD] getMicStream failed:', msg)
        vadMicErrorRef.current = msg
        throw err
      }
    },
    redemptionMs:   1500,  // 1500ms silence closes the speech segment
    minSpeechMs:     400,  // ignore utterances shorter than 400ms
    preSpeechPadMs:  150,  // keep 150ms before onset to avoid clipping first syllable
    onSpeechStart: () => {
      console.log('[VAD] speech start — state:', stateRef.current)
        ...
        try {
          ...
          if (resp!.ok) {
            ...
            await new Promise<void>((resolve) => {
              ...
              // Hard 7s cap — prevents any hang regardless of GStreamer/WebKit2GTK behavior
              const t = setTimeout(() => { console.warn('[SPEAKRESP] safety timeout fired'); cleanup(); safeResolve() }, 7_000)

              audio.onended = () => { console.log('[SPEAKRESP] audio ended naturally'); clearTimeout(t); cleanup(); safeResolve() }
              audio.onerror = () => { console.warn('[SPEAKRESP] audio onerror — falling back'); clearTimeout(t); cleanup(); safeResolve() }

              // Pre-prime at 85% — mic restarts before TTS fully ends
              audio.ontimeupdate = () => {
              ...
              // Web Audio API fallback — used when HTMLAudioElement silently fails
              // (WebKit2GTK: play() resolves but GStreamer never fires onended)
              const tryWebAudio = async () => {
                try {
                  // eslint-disable-next-line @typescript-eslint/no-explicit-any
                  const Cls = window.AudioContext ?? (window as any).webkitAudioContext
                  if (!Cls) { clearTimeout(t); cleanup(); safeResolve(); return }
                  const ctx = new (Cls as typeof AudioContext)()
                  // resume() with 500ms timeout — don't let it hang
                  await Promise.race([ctx.resume(), new Promise<void>(r => setTimeout(r, 500))])
                  // Use blob.arrayBuffer() — avoids CSP connect-src restrictions on blob: URLs
                  // that Tauri enforces (fetch(blobUrl) is blocked without 'blob:' in connect-src).
                  ...
                  src.onended = () => { clearTimeout(t); ctx.close().catch(() => {}); cleanup(); safeResolve() }
                  src.start(0)
                  // Pre-prime at 85% of duration
                  setTimeout(() => { if (!resolved) safeResolve() }, Math.max(0, buf.duration * 0.85) * 1000)
                  console.log('[SPEAKRESP] Web Audio playing — duration:', buf.duration.toFixed(2))
                } catch (waErr) {
                  console.warn('[SPEAKRESP] Web Audio fallback failed:', waErr)
              ...
              audio.play()
                .then(() => {
                  console.log('[SPEAKRESP] play() resolved')
                  // WebKit2GTK may resolve play() silently without actually playing.
                  // If loadedmetadata doesn't fire within 3s, GStreamer can't decode
                  // the format — fall back to Web Audio API immediately.
                  let metaLoaded = false
                  const metaTimer = setTimeout(() => {
                    if (!resolved && !metaLoaded) {
                      console.warn('[SPEAKRESP] loadedmetadata timeout — switching to Web Audio API')
                  ...
                  audio.onloadedmetadata = () => {
                    ...
                    console.log('[SPEAKRESP] metadata loaded — duration:', audio.duration?.toFixed(2))
                  }
                })
                .catch(async (err: unknown) => {
                  const errName = err instanceof Error ? err.name : String(err)
                  console.error('[SPEAKRESP] play() rejected:', errName)
                  void tryWebAudio()
                })
            })
            return
  ...
  _startListeningRef.current = async function _startListening(): Promise<void> {
    ...
    if (vad.errored) {
      console.error('[SESSION_FLOW] VAD errored (ONNX/SharedArrayBuffer issue):', vad.errored)
      setError('Voice engine failed to start — please refresh the page.')
      return
    }

    // Guard 6: stop immediately if a previous getUserMedia call failed.
    // The VAD instance enters a permanent internal "errored" state after
    // getUserMedia fails — subsequent start() calls silently no-op and the
    // session hangs in "listening" forever. Checking vadMicErrorRef prevents
    // this silent-success / deaf-session scenario.
    if (vadMicErrorRef.current) {
      console.error('[SESSION_FLOW] Mic previously failed:', vadMicErrorRef.current)
      const msg = /not found|devicenotfound/i.test(vadMicErrorRef.current)
        ? 'No microphone found — connect a mic and restart the session.'
        : /permission|notallowed/i.test(vadMicErrorRef.current)
          ? 'Microphone permission denied — allow mic access and restart.'
          : `Microphone unavailable (${vadMicErrorRef.current}) — restart the session.`
      setError(msg)
      activeRef.current = false
      setSessionActive(false)
      return
    ...
    try {
      ...
      // IMPORTANT: transition('idle') from 'listening' is BLOCKED by the state machine
      // (listening → [transcribing, stopped] only). Use forceIdle() here — it directly
      // sets stateRef.current = 'idle' without the ALLOWED check, allowing the retry
      // guard (which requires idle state) to work on the next maybeRestartListening call.
      const msg = err instanceof Error ? err.message : String(err)
      console.error('[SESSION_FLOW] vad.start() threw:', msg)
      forceIdle()   // bypasses state machine: sets state=idle + isRunning=false
      if (vadMicErrorRef.current) {
        // getUserMedia failed — VAD instance is permanently in "errored" state.
        // The next start() call would silently no-op and leave the session deaf.
        const friendly = /not found|devicenotfound/i.test(msg)
          ? 'No microphone found — connect a mic and restart the session.'
  ...
  const startSession = useCallback(async () => {
    // Module-level re-entrancy guard — React StrictMode double-mounts the hook, and the
    // wake-word callback fires on both instances simultaneously, producing 4× concurrent calls.
    if (_sessionStarting) {
      console.log('[SESSION] startSession re-entry blocked by lock')
      return
    }
    ...
    // Soft-state takeover: if a prior session is stuck in 'listening' (false wake),
    // tear it down so we can start fresh. Do NOT reset 'greeting' — another live
    // startSession() call owns that state; the lock above already guards against it.
    ...
    if (!activeRef.current) {
      _sessionStarting = false
      return  // stopped during greeting
    }
    transition('idle')
    _sessionStarting = false      // greeting done — allow new sessions
    console.log('[VOICE] session started — auto-starting listen')
    // 600ms gap: lets GStreamer release the audio output sink (TTS) before
    // getUserMedia opens the capture pipeline. Without this, simultaneous
    // output+input PulseAudio streams deadlock WebKit2GTK on WSL2.
    setTimeout(() => maybeRestartListening(), 600)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const stopSession = useCallback(() => {
    ...
    _sessionStarting       = false  // release lock so next startSession() can proceed
    activeRef.current      = false
    isStreamingRef.current = false
    isRunningRef.current   = false
    sessionIdRef.current   = ''
