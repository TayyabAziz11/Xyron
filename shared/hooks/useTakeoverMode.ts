...
// Tauri-aware base: relative URLs don't reach the backend from asset:// protocol
const _isTauri = typeof window !== 'undefined' && !!(window as unknown as Record<string, unknown>).__TAURI_INTERNALS__
const _apiBase  = typeof window === 'undefined' ? 'http://localhost:8000'
                : _isTauri ? 'http://localhost:8000' : ''
...
async function speakLine(text: string): Promise<void> {
  ...
  try {
    ...
    const t = setTimeout(() => ctrl.abort(), 4000)  // 4s timeout — skip line if TTS is slow
    let res: Response
    try {
      res = await fetch(`${_apiBase}/api/v1/voice/synthesize`, {
    ...
    await new Promise<void>((resolve) => {
      ...
      const guard   = setTimeout(() => { cleanup(); resolve() }, 15_000)
      a.onended  = () => { clearTimeout(guard); cleanup(); resolve() }
      a.onerror  = () => { clearTimeout(guard); cleanup(); resolve() }
      a.play().catch(() => { clearTimeout(guard); cleanup(); resolve() })
    })
  } catch { /* timeout or network error — skip this line */ }
}
...
export function useTakeoverMode() {
  ...
  // Sync with backend takeover state — polls every 10s as a fallback.
  // Primary trigger is the xyron:takeover / xyron:standdown events dispatched by useVoiceWS.
  // The poll only acts when the event-driven path missed the transition.
  useEffect(() => {
    let cancelled = false
    const poll = async () => {
      try {
        const res = await fetch(`${_apiBase}/api/v1/takeover/status`)
        if (!res.ok || cancelled) return
        const json = await res.json() as { active: boolean }
        if (json.active && !backendActiveRef.current && !runRef.current) {
          backendActiveRef.current = true
          console.log('[UI_MODE_CHANGE] mode=takeover source=backend_poll')
          void startTakeover()
        } else if (!json.active && backendActiveRef.current) {
          backendActiveRef.current = false
          console.log('[UI_MODE_CHANGE] mode=default source=backend_poll')
          void stopTakeover(false)
        }
  ...
  useEffect(() => {
    const onTakeover  = () => {
      backendActiveRef.current = true
      console.log('[UI_MODE_CHANGE] mode=takeover source=event')
      void startTakeover()
    }
    const onStanddown = () => {
      backendActiveRef.current = false
      console.log('[UI_MODE_CHANGE] mode=default source=event')
      void stopTakeover(true)
    }
